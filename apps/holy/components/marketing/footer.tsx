        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-6 h-6 rounded-md bg-[#7C3AED] flex items-center justify-center text-white font-bold text-xs">H</div>
          ...
          {[
            { title: 'Product', links: [['Features', '#features'], ['Pricing', '/pricing'], ['Store', '/dashboard/store'], ['Dashboard', '/dashboard']] },
            { title: 'Company', links: [['About', '#'], ['Blog', '#'], ['Careers', '#'], ['Contact', '#']] },
            { title: 'Legal', links: [['Privacy', '#'], ['Terms', '#'], ['Licenses', '#']] },
          ].map(({ title, links }) => (
            <div key={title}>
              <h4 className="text-xs font-semibold uppercase tracking-wider text-[#A1A1AA] mb-4">{title}</h4>
