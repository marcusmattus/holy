...
const PAYOUTS = [
  { id: '1', amount: 124.50, date: '2025-01-15', status: 'completed', txHash: '0xabc...def' },
  { id: '2', amount: 89.20, date: '2025-01-01', status: 'completed', txHash: '0x123...456' },
  { id: '3', amount: 210.00, date: '2024-12-15', status: 'completed', txHash: '0x789...abc' },
]
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <Link href="/dashboard/revenue" className="hover:text-foreground">Revenue</Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-xs text-muted-foreground mt-1">via on-chain settlements</p>
        </div>
      </div>
          ...
          <tbody>
            {PAYOUTS.map((p) => (
              <tr key={p.id} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-3 text-[#7C3AED] font-mono text-xs">{p.txHash}</td>
